...
if (isset($_GET['token'])) {
	...
	$sql = "SELECT verified, token FROM logintb WHERE verified=0 AND token='$token' LIMIT 1";
	...
	$num = mysqli_num_rows($result);
	  ...
	  if($num ==1){
	  	...
	  	$update = "UPDATE logintb SET verified= 1 WHERE token='$token' LIMIT 1";
	...
	header('Location: confirm-email?error=error');
}
